...
if len(sys.argv) != 3:
    print(f"{PREFIX} Usage: generate-ai-lead.py <article_url> <openai_api_key>")
    sys.exit(1)
...
system_content = "あなたは技術記事のプロモーション専門家です。指定されたURLの記事を読み取って、X（Twitter）で人々がクリックしたくなる魅力的なリード文を生成してください。要件: 90文字以内。技術的な価値を強調、読者の興味を引く表現、ハッシュタグは含めない、記事の具体的なメリットを明示。"
...
log(f"Final lead text: {LEAD_TEXT}")
